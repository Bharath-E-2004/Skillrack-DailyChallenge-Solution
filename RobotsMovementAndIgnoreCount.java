...
public class Hello {
    ...
    public static void main(String[] args) {
        ...
        int ignored = 0;
        int pos = 0;
        int freeTime = 0; // time when robot becomes free
        ...
        for (int i = 0; i < N; i++) {
            ...
            // If robot is still moving, ignore command
            if (T < freeTime) {
                ignored++;
            } 
            // Robot is free → accept command
            else {
                int distance = Math.abs(X - pos);
                freeTime = T + distance;
                pos = X;
            }
        }
...
t = 7 the position of the robot is 4.
