...
public class Hello {
    public static void main(String[] args) {
        ...
        String str = sc.next();   // input string of digits
        int k = sc.nextInt();     // number of iterations

        for (int t = 1; t <= k; t++) {
            for (int i = 0; i < str.length(); i += 3) {
                // take 3-digit block (pad if needed at the end)
                int end = Math.min(i + 3, str.length());
                String block = str.substring(i, end);

                int val = Integer.parseInt(block) + i;
                String s = String.valueOf(val);
                int len = s.length();

                if (len < 3) {
                    // pad with leading zeros
                    System.out.printf("%s", "0".repeat(3 - len) + s);
                } else {
                    // take last 3 digits
                    System.out.printf("%s", s.substring(len - 3));
                }
            }
            System.out.print(" ");
        }
...
- Every three digits in the odometer represent a value or parameter. - Every second, every 3rd digit in the odometer increases by 1.
...
Here the given string is 655087291 and the value of K is 15. The states of the odometer for the next 15 seconds are printed.
